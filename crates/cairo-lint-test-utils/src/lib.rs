    ...
    ($lint_group: ident, $file_path:ident, $($test_name:expr),*) => {
        ...
        paste ! {
            ...
            fn [<$lint_group _ $file_path>](test_name: &str) {
                ...
                assert!(&formatted_diags, test.attributes.get("diagnostics").expect("Couldn't get expected diagnostics"));
                assert!(&file, test.attributes.get("fixed").expect("Couldn't get expected fix"));
            }
        }
    };
}
