...
pub fn check_double_comparison(db: &dyn SyntaxGroup, binary_expr: &ExprBinary, diagnostics: &mut Vec<PluginDiagnostic>) {
    let lhs = binary_expr.lhs(db);
    let rhs = binary_expr.rhs(db);
    let middle_op = binary_expr.op(db);

    if let (Some(lhs_op), Some(rhs_op)) = (extract_binary_operator_expr(&lhs, db), extract_binary_operator_expr(&rhs, db)) {
...
fn is_redundant_double_comparison(lhs_op: &BinaryOperator, rhs_op: &BinaryOperator, middle_op: &BinaryOperator) -> bool {
    matches!(
