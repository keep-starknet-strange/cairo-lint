...
pub fn check_collapsible_if(
    ...
    if if_block.statements.len() == 1
        && if_block.tail.is_none()
        // If the inner statement is an expression
        && let Statement::Expr(ref inner_expr_stmt) = arenas.statements[if_block.statements[0]]
        // And this expression is an if expression
        && let Expr::If(ref inner_if_expr) = arenas.exprs[inner_expr_stmt.expr]
    {
    ...
    } else if if_block.tail.is_some_and(|tail| {
