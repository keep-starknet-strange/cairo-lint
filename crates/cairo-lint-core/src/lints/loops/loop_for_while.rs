...
fn check_if_contains_break(expr: &ExprId, arenas: &Arenas) -> bool {
    ...
    // Get the block
        && let Expr::Block(ref if_block) = arenas.exprs[if_expr.if_block]
        // Get the first statement of the if
        && let Some(inner_stmt) = if_block.statements.first()
        // Is it a break statement 
        && matches!(arenas.statements[*inner_stmt], Statement::Break(_))
    {
        true
    } else {
        false
