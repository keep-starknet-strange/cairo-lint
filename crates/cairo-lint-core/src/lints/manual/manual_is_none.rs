...
pub fn check_manual_is_none(db: &dyn SyntaxGroup, expr_match: &ExprMatch, diagnostics: &mut Vec<PluginDiagnostic>) {
    ...
    // Checks if the pattern matches Option::Some(_) => false
    let found_some_ok = match first_pattern {
        Pattern::Enum(enum_pattern) => {
            ...
            match enum_name.as_str() {
                "Option::Some" => first_arm.expression(db).as_syntax_node().get_text_without_trivia(db) == "false",
                _ => false,
            }
        }
        _ => false,
    ...
    // Checks if the pattern matches Option::None => true
    let found_none_err = match second_pattern {
        Pattern::Enum(enum_pattern) => {
            let enum_name = enum_pattern.path(db).as_syntax_node().get_text_without_trivia(db);
            match enum_name.as_str() {
                "Option::None" => second_arm.expression(db).as_syntax_node().get_text_without_trivia(db) == "true",
                _ => false,
            }
        }
        _ => false,
